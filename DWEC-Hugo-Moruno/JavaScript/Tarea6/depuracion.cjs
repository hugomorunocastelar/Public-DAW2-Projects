...
const iniciales = ' HMP';//read("¿Cuáles son tus iniciales?");
...
let tamaño = 1378n; // <-- Esta variable tiene que ser el tamaño del array
let n = 0n;
for(;n < tamaño;n++) {
    vector[n] = n%128n;
}
...
function reto6() {
    ...
    for(;n < iteraciones;n++) { if(n == iteracion && resultado != valor_anterior) throw "RETO 6 No superado"; else if(n == iteracion) break; else valor_anterior = BigInt(Math.random().toFixed()); }
        //added conditional breakpoint to { n == iteracion - 1n; }
